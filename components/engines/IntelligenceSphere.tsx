import { missionSignals } from "@/data/assets";
...
export function IntelligenceSphere() {
  ...
  return (
    <div className="relative mx-auto h-[560px] w-full max-w-5xl overflow-hidden rounded-[2rem] border border-line bg-obsidian shadow-glow">
      ...
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 980 560" aria-hidden="true">
        <defs>
          <radialGradient id="coreGlow">
            ...
            <stop offset="35%" stopColor="#38BDF8" stopOpacity=".55" />
            <stop offset="100%" stopColor="#38BDF8" stopOpacity="0" />
          </radialGradient>
          <linearGradient id="sphereFlow" x1="0" x2="1">
            <stop offset="0%" stopColor="#38BDF8" stopOpacity=".2" />
            <stop offset="50%" stopColor="#38BDF8" stopOpacity=".9" />
            <stop offset="100%" stopColor="#F59E0B" stopOpacity=".8" />
          </linearGradient>
        </defs>

        <circle cx="490" cy="280" r="96" fill="url(#coreGlow)" opacity=".8" />
        <circle cx="490" cy="280" r="8" fill="#F8FAFC" />
        <path d="M150 190 C 290 90, 445 165, 490 280 S 735 430, 860 270" fill="none" stroke="url(#sphereFlow)" strokeWidth="3" strokeDasharray="12 10" className="risk-line" />
      ...
      <div className="absolute bottom-6 left-6 right-6 grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl border border-greenSafe/30 bg-greenSafe/10 p-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-greenSafe">PTR change</p>
          <p className="mt-2 text-2xl font-semibold text-white">72 → 41</p>
        </div>
        <div className="rounded-2xl border border-amberRisk/30 bg-amberRisk/10 p-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-amberRisk">Revenue protected</p>
          <p className="mt-2 text-2xl font-semibold text-white">$87K</p>
        </div>
        <div className="rounded-2xl border border-blueIntel/30 bg-blueIntel/10 p-4">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-blueIntel">Confidence</p>
          <p className="mt-2 text-2xl font-semibold text-white">96.8%</p>
        </div>
      </div>
    </div>
  );
