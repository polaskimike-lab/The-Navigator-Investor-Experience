...
export function ExecutiveSimulator() {
  ...
  const model = useMemo(() => {
    ...
    const annualRevenueProtected = 184.2 * scale * weatherFactor * shipmentFactor * accidentFactor;
    const fuelSavings = 18.4 * scale * dieselFactor * mileFactor * (0.75 + emptyMileFactor * 0.25);
    ...
    return { annualRevenueProtected, fuelSavings, maintenanceAvoidance, operatingSavings, implementationCost, paybackMonths, threeYearRoi, ptrStart, ptrEnd, confidence };
    ...
    <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <div className="mission-panel rounded-[2rem] p-7">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-amberRisk/25 bg-amberRisk/10 p-5"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">Estimated Annual Revenue Protected</p><p className="mt-3 text-4xl font-semibold tracking-[-0.06em] text-amberRisk">{money(model.annualRevenueProtected)}</p></div>
          <div className="rounded-2xl border border-greenSafe/25 bg-greenSafe/10 p-5"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">Estimated Annual Operating Savings</p><p className="mt-3 text-4xl font-semibold tracking-[-0.06em] text-greenSafe">{money(model.operatingSavings)}</p></div>
          <div className="rounded-2xl border border-line bg-black/25 p-5"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">Fleet Fuel Savings</p><p className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-white">{money(model.fuelSavings)}</p></div>
          <div className="rounded-2xl border border-line bg-black/25 p-5"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">Maintenance Avoidance</p><p className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-white">{money(model.maintenanceAvoidance)}</p></div>
          <div className="rounded-2xl border border-blueIntel/25 bg-blueIntel/10 p-5"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">Estimated Payback Period</p><p className="mt-3 text-4xl font-semibold tracking-[-0.06em] text-blueIntel">{model.paybackMonths.toFixed(1)} mo</p></div>
          <div className="rounded-2xl border border-blueIntel/25 bg-blueIntel/10 p-5"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">Estimated 3-Year ROI</p><p className="mt-3 text-4xl font-semibold tracking-[-0.06em] text-blueIntel">{Math.round(model.threeYearRoi)}%</p></div>
        </div>
        <div className="mt-5 rounded-3xl border border-line bg-black/25 p-5">
          <div className="grid gap-4 md:grid-cols-3">
            <div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted">PTR</p><p className="mt-2 text-2xl font-semibold text-greenSafe">{model.ptrStart} → {model.ptrEnd}</p></div>
